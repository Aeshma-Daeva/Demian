...
def render_neuron_svg(trace: dict[str, Any]) -> str:
    ...
    parts = [
        ...
        '<text x="34" y="42" font-family="Arial, sans-serif" font-size="23" font-weight="700" fill="#202124">Neuron activations, quadratic color</text>',
        f'<text x="34" y="70" font-family="Arial, sans-serif" font-size="13" fill="#4c535a">v9 five-channel, seed={config["seed"]}, hidden={hidden_size}, steps={steps}; teal=positive, red=negative, intensity=(|x|/max)^2</text>',
    ]
    for channel_index, name in enumerate(CHANNELS):
        y0 = top + channel_index * (panel_h + panel_gap)
        parts.append(
            f'<text x="34" y="{y0 + label_gap}" font-family="Arial, sans-serif" font-size="13" font-weight="700" fill="#202124">{escape_xml(name)}</text>'
        )
    ...
    parts.extend(
        [
            ...
            f'<text x="54" y="{legend_y}" font-family="Arial, sans-serif" font-size="12" fill="#4c535a">negative max</text>',
            f'<rect x="160" y="{legend_y - 10}" width="12" height="12" fill="{signed_quadratic_color(0.0, max_abs)}"/>',
            f'<text x="180" y="{legend_y}" font-family="Arial, sans-serif" font-size="12" fill="#4c535a">near zero</text>',
            f'<rect x="270" y="{legend_y - 10}" width="12" height="12" fill="{signed_quadratic_color(max_abs, max_abs)}"/>',
            f'<text x="290" y="{legend_y}" font-family="Arial, sans-serif" font-size="12" fill="#4c535a">positive max</text>',
            "</svg>",
        ]
    )
    return "\n".join(parts) + "\n"
...
def render_gate_svg(trace: dict[str, Any]) -> str:
    ...
    parts = [
        ...
        f'<rect width="{width}" height="{height}" fill="#f7f7f4"/>',
        '<text x="34" y="42" font-family="Arial, sans-serif" font-size="23" font-weight="700" fill="#202124">Gating activations, quadratic color</text>',
        f'<text x="34" y="70" font-family="Arial, sans-serif" font-size="13" fill="#4c535a">v9 five-channel, seed={config["seed"]}, hidden={config["hidden_size"]}, steps={steps}; intensity=(metric/max)^2</text>',
    ]
    for row_index, (name, label) in enumerate(GATES):
        ...
        parts.append(
            f'<text x="34" y="{y + 11}" font-family="Arial, sans-serif" font-size="12" font-weight="700" fill="#202124">{escape_xml(label)}</text>'
        )
    ...
    parts.extend(
        [
            ...
            f'<text x="54" y="{legend_y}" font-family="Arial, sans-serif" font-size="12" fill="#4c535a">low</text>',
            f'<rect x="104" y="{legend_y - 10}" width="12" height="12" rx="1" fill="{unsigned_quadratic_color(0.5, 1.0)}"/>',
            f'<text x="124" y="{legend_y}" font-family="Arial, sans-serif" font-size="12" fill="#4c535a">mid maps to 25% intensity</text>',
            f'<rect x="310" y="{legend_y - 10}" width="12" height="12" rx="1" fill="{unsigned_quadratic_color(1.0, 1.0)}"/>',
            f'<text x="330" y="{legend_y}" font-family="Arial, sans-serif" font-size="12" fill="#4c535a">high</text>',
            "</svg>",
        ]
    )
    return "\n".join(parts) + "\n"
